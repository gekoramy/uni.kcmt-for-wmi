...
from wmpy.enumeration.sae import AssignmentExtractor
...
try:
    ...
    raise ImportError("MathSAT is not installed. Please install it using the `wmpy install` command.") from e
        ...
        if len(bool_atoms) == 0:
            ...
            for ta_bool in mus_projected:
                ...
                if is_convex:
                    ...
                    # simplified formula is non-convex, requiring another enumeration pass
                    residual_atoms = list(
                        {
                            a
