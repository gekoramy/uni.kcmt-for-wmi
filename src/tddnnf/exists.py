...
def bcs12(args: ArgumentsWithBCS12):
    match len(args.project_onto):
        ...
        case n if n == abstraction.atoms(args.mapping):
    ...
    match len(args.project_onto):
        ...
        case _:
            ...
            with utils.log('projecting'):
                which: list[int] = [1] * (1 + abstraction.atoms(args.mapping))
...
def main() -> None:
    ...
    with utils.log('total'):
        ...
        mapping: abstraction.i2atom = abstraction.read_mapping(env=env, mapping=args.mapping)
        ...
        project_onto: list[int] = [k for k, v in abstraction.entries(mapping) if to_project_onto(v)]
